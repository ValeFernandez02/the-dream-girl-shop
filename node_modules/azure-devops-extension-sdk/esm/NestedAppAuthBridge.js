 ...
 */
async function initializeNestedAppAuthBridge(parentChannel) {
    ...
    try {
        await parentChannel.invokeRemoteMethod("processRequest", nestedAppAuthHostId, [
            JSON.stringify({ messageType: "NestedAppAuthRequest", method: "Ping", requestId: "naa-init-ping" })
        ]);
    }
    catch (err) {
        throw new Error("Nested App Authentication is not available in this Azure DevOps environment yet. " +
    ...
    const bridge = {
        ...
        postMessage(requestJson) {
            parentChannel.invokeRemoteMethod("processRequest", nestedAppAuthHostId, [requestJson]).then((responseJson) => {
    ...
    window.nestedAppAuthBridge = bridge;
}
/**
 ...
 */
function teardownNestedAppAuthBridge() {
    ...
    bridge.postMessage = function (requestJson) {
        try {
            const request = JSON.parse(requestJson);
            let response;
...
export { initializeNestedAppAuthBridge, teardownNestedAppAuthBridge };
//# sourceMappingURL=NestedAppAuthBridge.js.map
